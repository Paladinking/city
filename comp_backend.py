...
def _box2d_hook(path: pathlib.Path, pkg: Dict[str, Any]) -> None:
    ...
    cmake_args = [cmake, '-B', str(build_dir), '-S', str(cmake_src_path),
                  '-G', 'Ninja', '-DCMAKE_BUILD_TYPE=Release', 
                  f'-DCMAKE_INSTALL_PREFIX={path}', '-DBOX2D_SAMPLES=OFF', 
                  '-DBOX2D_UNIT_TESTS=OFF', '-DBUILD_SHARED_LIBS=OFF']

    if backend().name == "emcc":
        emcmake = shutil.which('emcmake.bat')
        if emcmake is None:
            raise RuntimeError("Could not find emcmake.bat")
        cmake_args = [emcmake] + cmake_args
    elif backend().name == "msvc":
        cmake_args.append("-DCMAKE_C_COMPILER=cl.exe")
    else:
        cmake_args.append("-DCMAKE_C_COMPILER=gcc.exe")
